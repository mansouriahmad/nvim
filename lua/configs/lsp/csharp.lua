...
-- Find alternative C# language servers
local function find_csharp_ls()
  local mason_path = vim.fn.stdpath("data") .. "/mason/bin"
  local paths = { mason_path }
  
  if get_platform() == "windows" then
    return find_executable("csharp-ls.exe", paths)
  else
    return find_executable("csharp-ls", paths)
  end
end
...
function M.setup_lsp(capabilities)
  ...
  if omnisharp_cmd then
    ...
    -- Fallback to csharp-ls if available
    local csharp_ls_cmd = find_csharp_ls()
    if csharp_ls_cmd then
      vim.notify("OmniSharp not found, using csharp-ls at: " .. csharp_ls_cmd, vim.log.levels.WARN)
      ...
      vim.notify("No C# language server found. Install via :MasonInstall omnisharp", vim.log.levels.ERROR)
    end
  end
end
